...
const MatchSetup = ({ isAdmin: isAdminProp, onTeamSelect, onMatchSelect }) => {
  const { user } = useUser();
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      ...
      try {
        // Query Match_Info_ADMIN collection directly for fastest retrieval
        // Use provider=admin to access dedicated admin collection
        const res = await fetch('/api/matches?provider=admin&limit=50&range=30&includePast=1');
        const json = await res.json();
        
        console.log('[MatchSetup] API Response:', {
          success: json.success,
          provider: json.provider,
          count: json.data?.length,
          matches: json.data?.map(m => ({ id: m.id, home: m.homeTeam?.name || m.homeTeam, away: m.awayTeam?.name || m.awayTeam }))
        });
        ...
        // Filter for matches within the time window
        ...
        console.log('[MatchSetup] Filtered matches:', upcoming.length);
        setMatches(upcoming);
      } catch (err) {
  ...
  const validateMatch = (match) => {
    ...
    // Matchday is optional, but if provided, validate it
    if (match.matchday !== '' && match.matchday !== null && match.matchday !== undefined) {
      if (isNaN(Number(match.matchday)) || Number(match.matchday) < 1) {
  ...
  const handleMatchSelect = (match) => {
    ...
    // Notify parent Dashboard so the Live Input tab appears in Header
    if (onMatchSelect) {
      onMatchSelect(match);
    }
  };
  ...
  return (
    <div className="ms-container">
      {/* Toast Notifications */}
      {successMessage && (
        <div className={`ms-toast ${successMessage.includes('Failed') ? 'ms-error' : 'ms-success'}`}>
        ...
        <div className="ms-modal-overlay">
          <div className="ms-modal">
            <div className="ms-modal-header">
              <h3>Remove Match</h3>
              <button className="ms-modal-close" onClick={cancelRemove}>×</button>
            </div>
            
            <div className="ms-modal-content">
              <div className="ms-preview">
                <div className="ms-preview-teams">
                  <div className="ms-preview-team ms-home">
                    <img 
                  ...
                  <div className="ms-preview-vs">vs</div>
                  
                  <div className="ms-preview-team ms-away">
                    <img 
                ...
                <div className="ms-preview-details">
                  <div className="ms-preview-detail">
                    <strong>Date:</strong> {removeConfirm.match.date} {removeConfirm.match.time}
                  </div>
                  <div className="ms-preview-detail">
                    <strong>Competition:</strong> {getLeagueName(removeConfirm.match.competition?.name || removeConfirm.match.competition?.code || removeConfirm.match.competitionName || '')}
                  </div>
              ...
              <p className="ms-warning-text">This action cannot be undone. The match will be permanently removed.</p>
            </div>
            
            <div className="ms-modal-actions">
              <button className="ms-btn-secondary" onClick={cancelRemove}>
                Cancel
              </button>
              <button className="ms-btn-danger" onClick={confirmRemove}>
                Remove Match
              </button>
            </div>
          </div>
      ...
      {selectedMatch ? (
        <MatchViewer match={selectedMatch} initialSection="details" onBack={() => setSelectedMatch(null)} isAdmin={isAdmin} />
      ) : (
        <>
          ...
          {showForm && (
            <div className="ms-form">
              ...
              <div className="ms-bulk-import">
                ...
                {showFileImport && (
                  <div className="ms-file-import-panel">
                    ...
                    {importProgress.total > 0 && (
                      <div className="ms-import-progress">
                        ...
                        <div className="ms-progress-text">
              ...
              <div className="ms-form-row">
                <div className="ms-form-field">
                  <select
                    ...
                    className={errors.teamA ? 'ms-error' : ''}
                  >
                    <option value="">Select Home Team</option>
                    {teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}
                  </select>
                  {errors.teamA && <span className="ms-error-message">{errors.teamA}</span>}
                </div>
                ...
                <div className="ms-form-field">
                  <select
                    ...
                    className={errors.teamB ? 'ms-error' : ''}
                  >
                    <option value="">Select Away Team</option>
                    {teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}
                  </select>
                  {errors.teamB && <span className="ms-error-message">{errors.teamB}</span>}
                </div>
              </div>
              ...
              <div className="ms-form-row">
                <div className="ms-form-field">
                  <input
                    ...
                    className={errors.date ? 'ms-error' : ''}
                  />
                  {errors.date && <span className="ms-error-message">{errors.date}</span>}
                </div>
                ...
                <div className="ms-form-field">
                  <input
                    ...
                    className={errors.time ? 'ms-error' : ''}
                  />
                  {errors.time && <span className="ms-error-message">{errors.time}</span>}
                </div>
              </div>
              ...
              <div className="ms-form-row">
                <div className="ms-form-field">
                  <select 
                    ...
                    className={errors.competition ? 'ms-error' : ''}
                  >
                    <option value="">Select Competition</option>
                    {competitions.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  {errors.competition && <span className="ms-error-message">{errors.competition}</span>}
                </div>
                ...
                <div className="ms-form-field">
                  <input
                    ...
                    name="matchday"
                    min="1"
                    placeholder="Matchday (optional)"
                    value={newMatch.matchday}
                    onChange={(e) => {
                      handleInputChange(e);
                      clearErrors('matchday');
                    }}
                    className={errors.matchday ? 'ms-error' : ''}
                    style={{ width: '140px' }}
                  />
                  {errors.matchday && <span className="ms-error-message">{errors.matchday}</span>}
                </div>
              </div>
                ...
                {matches.map(match => {
                  ...
                  return (
                    <div
                      ...
                      className={`ms-match-card ${match._optimistic ? 'ms-optimistic' : ''}`}
                      onClick={() => handleMatchSelect(match)}
                    >
                      <div className="ms-match-header">
                        <div className="ms-match-competition">
                          {getLeagueName(match.competition?.name || match.competition?.code || match.competitionName || '')}
                        </div>
